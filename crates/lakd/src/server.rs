...
use lak_core::types::agent::{
    Agent as CoreAgent, AgentConfig, AgentSpec, AgentState,
};
use lak_core::types::capability::{
    Capability, CapabilityPermission, CapabilityRequirement,
    CapabilityScope, CapabilityType,
};
...
fn agent_id_to_bytes(id: AgentId) -> Vec<u8> { id.as_uuid().as_bytes().to_vec() }
fn task_id_to_bytes(id: TaskId) -> Vec<u8> { id.as_uuid().as_bytes().to_vec() }
fn intent_id_to_bytes(id: IntentId) -> Vec<u8> { id.as_uuid().as_bytes().to_vec() }
fn memory_chunk_id_to_bytes(id: MemoryChunkId) -> Vec<u8> { id.as_uuid().as_bytes().to_vec() }
fn capability_cert_id_to_bytes(id: CapabilityCertId) -> Vec<u8> { id.as_uuid().as_bytes().to_vec() }
...
fn task_state_from_pb(s: i32) -> TaskState {
    match pb::TaskState::try_from(s) {
        ...
        Ok(pb::TaskState::Failed) => TaskState::Failed(TaskError {
            code: "UNKNOWN".into(), message: "".into(), retryable: false,
        }),
...
fn intent_target_from_pb(t: &Option<pb::IntentTarget>) -> Result<IntentTarget, Status> {
    let t = t.as_ref().ok_or_else(|| Status::invalid_argument("missing intent target"))?;
    ...
    match tt {
        ...
        pb::IntentTargetType::IntentTargetUnicast => {
            let id = t.agent_ids.first()
                .map(|b| bytes_to_agent_id(b)).transpose()?
                .unwrap_or(AgentId::SYSTEM);
            Ok(IntentTarget::Unicast(id))
        }
        pb::IntentTargetType::IntentTargetMulticast => {
            let ids: Result<Vec<AgentId>, Status> = t.agent_ids.iter()
                .map(|b| bytes_to_agent_id(b)).collect();
...
fn agent_spec_from_pb(pb_spec: &pb::AgentSpec) -> AgentSpec {
    ...
    AgentSpec {
        ...
        initial_capabilities: pb_spec.initial_capabilities.iter().map(|c| Capability {
            cap_type: cap_type_from_pb(c.cap_type),
            scope: CapabilityScope { pattern: c.scope.clone() },
        ...
        config: AgentConfig {
            ...
            require_approval_for_write: config.map(|c| c.require_approval_for_write).unwrap_or(false),
...
fn cognitive_task_from_pb(pb_task: &pb::CognitiveTask) -> Result<CognitiveTask, Status> {
    Ok(CognitiveTask {
        ...
        priority: CognitivePriority {
            urgency: pb_task.priority.as_ref().map(|p| p.urgency as u8).unwrap_or(40),
            importance: pb_task.priority.as_ref().map(|p| p.importance as u8).unwrap_or(50),
            context_affinity: pb_task.priority.as_ref().map(|p| p.context_affinity as u8).unwrap_or(50),
        },
        state: task_state_from_pb(pb_task.state),
        content: TaskContent {
            natural_language: pb_task.content.as_ref().map(|c| c.natural_language.clone()).unwrap_or_default(),
            structured_schema: pb_task.content.as_ref()
                .and_then(|c| c.structured_schema_json.as_ref())
                .and_then(|s| serde_json::from_str(s).ok()),
            memory_references: pb_task.content.as_ref()
                .map(|c| c.memory_references.iter().filter_map(|b| bytes_to_memory_chunk_id(b).ok()).collect())
                .unwrap_or_default(),
        },
        deadline: pb_task.deadline.as_ref().and_then(|ts| timestamp_to_datetime(Some(ts)).ok()),
        dependencies: pb_task.dependencies.iter().filter_map(|b| bytes_to_task_id(b).ok()).collect(),
        created_at: pb_task.created_at.as_ref().and_then(|ts| timestamp_to_datetime(Some(ts)).ok()).unwrap_or_else(Utc::now),
        updated_at: pb_task.updated_at.as_ref().and_then(|ts| timestamp_to_datetime(Some(ts)).ok()).unwrap_or_else(Utc::now),
...
fn cognitive_task_to_pb(task: &CognitiveTask) -> pb::CognitiveTask {
    pb::CognitiveTask {
        ...
        content: Some(pb::TaskContent {
            ...
            structured_schema_json: task.content.structured_schema.as_ref().map(|v| v.to_string()),
            memory_references: task.content.memory_references.iter().map(|id| memory_chunk_id_to_bytes(*id)).collect(),
        }),
        deadline: opt_datetime_to_timestamp(task.deadline),
        dependencies: task.dependencies.iter().map(|id| task_id_to_bytes(*id)).collect(),
        ...
        error: match &task.state {
            TaskState::Failed(e) => Some(pb::TaskErrorProto {
                code: e.code.clone(), message: e.message.clone(), retryable: e.retryable,
            }),
            _ => None,
        },
    }
...
fn intent_message_from_pb(pb_intent: &pb::IntentMessage) -> Result<IntentMessage, Status> {
    Ok(IntentMessage {
        ...
        content: IntentContent {
            natural_language: pb_intent.content.as_ref().map(|c| c.natural_language.clone()).unwrap_or_default(),
            structured_data: pb_intent.content.as_ref()
        ...
        ttl_ms: 30_000,
        correlation_id: pb_intent.reply_to_intent_id.as_ref()
            .and_then(|b| bytes_to_intent_id(b).ok()),
        created_at: pb_intent.created_at.as_ref()
...
fn intent_message_to_pb(intent: &IntentMessage) -> pb::IntentMessage {
    pb::IntentMessage {
        ...
        target: None,
        intent_type: pb::IntentType::Notification as i32,
        content: Some(pb::IntentContent {
            natural_language: intent.content.natural_language.clone(),
            structured_json: intent.content.structured_data.as_ref().map(|v| v.to_string()),
            headers: std::collections::HashMap::new(),
        }),
        priority: Some(pb::CognitivePriority { urgency: 40, importance: 50, context_affinity: 50 }),
        reply_to_intent_id: intent.correlation_id.map(|id| intent_id_to_bytes(id)),
        requires_ack: false,
...
fn intent_subscription_from_pb(pb_sub: &pb::IntentSubscription) -> IntentSubscription {
    IntentSubscription {
        agent_id: AgentId::new(),
        intent_types: if pb_sub.intent_types.is_empty() {
            None
        } else {
            Some(pb_sub.intent_types.iter().map(|t| intent_type_from_pb(*t)).collect())
        },
...
fn memory_chunk_from_pb(pb_chunk: &pb::MemoryChunk) -> Result<MemoryChunk, Status> {
    Ok(MemoryChunk {
        ...
        content: MemoryContent {
            raw_text: pb_chunk.content.as_ref().map(|c| c.raw_text.clone()).unwrap_or_default(),
            structured_data: None,
            embedding: pb_chunk.content.as_ref()
                .and_then(|c| c.embedding_json.as_ref())
                .and_then(|s| serde_json::from_str(s).ok()),
        },
        metadata: MemoryMetadata {
            created_at: pb_chunk.created_at.as_ref()
                .and_then(|ts| timestamp_to_datetime(Some(ts)).ok()).unwrap_or_else(Utc::now),
            last_accessed_at: pb_chunk.last_accessed_at.as_ref()
                .and_then(|ts| timestamp_to_datetime(Some(ts)).ok()).unwrap_or_else(Utc::now),
            access_count: pb_chunk.access_count as u64,
            importance_score: pb_chunk.metadata.as_ref().map(|m| m.importance_score as f32).unwrap_or(0.5),
            decay_rate: 0.01,
            source: pb_chunk.metadata.as_ref()
                .map(|m| memory_source_from_pb(m.source))
                .unwrap_or(MemorySource::ExternalSource),
            factuality: pb_chunk.metadata.as_ref()
...
fn memory_chunk_to_pb(chunk: &MemoryChunk) -> pb::MemoryChunk {
    pb::MemoryChunk {
        ...
        content: Some(pb::MemoryContentProto {
            ...
            embedding_json: chunk.content.embedding.as_ref()
                .map(|e| serde_json::to_string(e).unwrap_or_default()),
            tags: std::collections::HashMap::new(),
        }),
        metadata: Some(pb::MemoryMetadataProto {
            source: pb::MemorySourceType::MemorySourceObservation as i32,
            factuality: pb::Factuality::Belief as i32,
            confidence: None,
...
impl GrpcAgentKernel for LakGrpcServer {
    ...
    async fn create_agent(
        &self, request: Request<pb::CreateAgentRequest>,
        ...
        let agent_id = self.kernel.create_agent(agent_spec).await.map_err(kernel_error_to_status)?;
        Ok(Response::new(pb::CreateAgentResponse { agent_id: agent_id_to_bytes(agent_id) }))
    }

    async fn destroy_agent(
        &self, request: Request<pb::DestroyAgentRequest>,
    ) -> Result<Response<()>, Status> {
        let req = request.into_inner();
        let agent_id = bytes_to_agent_id(&req.agent_id)?;
        self.kernel.destroy_agent(agent_id).await.map_err(kernel_error_to_status)?;
        Ok(Response::new(()))
    }

    async fn get_agent(
        &self, request: Request<pb::GetAgentRequest>,
    ) -> Result<Response<pb::GetAgentResponse>, Status> {
        let req = request.into_inner();
        let agent_id = bytes_to_agent_id(&req.agent_id)?;
        let agent = self.kernel.get_agent(agent_id).await.map_err(kernel_error_to_status)?;
        Ok(Response::new(pb::GetAgentResponse { agent: Some(agent_to_pb(&agent)) }))
    }

    async fn list_agents(
        &self, _request: Request<()>,
    ) -> Result<Response<pb::ListAgentsResponse>, Status> {
        let agents = self.kernel.list_agents().await.map_err(kernel_error_to_status)?;
    ...
    async fn pause_agent(
        &self, request: Request<pb::PauseAgentRequest>,
    ) -> Result<Response<()>, Status> {
        let req = request.into_inner();
        self.kernel.pause_agent(bytes_to_agent_id(&req.agent_id)?)
            .await.map_err(kernel_error_to_status)?;
        Ok(Response::new(()))
    }

    async fn resume_agent(
        &self, request: Request<pb::ResumeAgentRequest>,
    ) -> Result<Response<()>, Status> {
        let req = request.into_inner();
        self.kernel.resume_agent(bytes_to_agent_id(&req.agent_id)?)
            .await.map_err(kernel_error_to_status)?;
        Ok(Response::new(()))
    }
    ...
    async fn submit_task(
        &self, request: Request<pb::SubmitTaskRequest>,
    ) -> Result<Response<pb::SubmitTaskResponse>, Status> {
        let req = request.into_inner();
        let task_pb = req.task.as_ref()
            .ok_or_else(|| Status::invalid_argument("missing task"))?;
        let task = cognitive_task_from_pb(task_pb)?;
        let task_id = self.kernel.submit_task(task).await.map_err(kernel_error_to_status)?;
        Ok(Response::new(pb::SubmitTaskResponse { task_id: task_id_to_bytes(task_id) }))
    }

    async fn cancel_task(
        &self, request: Request<pb::CancelTaskRequest>,
    ) -> Result<Response<()>, Status> {
        let req = request.into_inner();
        self.kernel.cancel_task(bytes_to_task_id(&req.task_id)?)
            .await.map_err(kernel_error_to_status)?;
        Ok(Response::new(()))
    }

    async fn get_task(
        &self, request: Request<pb::GetTaskRequest>,
    ) -> Result<Response<pb::GetTaskResponse>, Status> {
        let req = request.into_inner();
        let task = self.kernel.get_task(bytes_to_task_id(&req.task_id)?)
            .await.map_err(kernel_error_to_status)?;
        Ok(Response::new(pb::GetTaskResponse { task: Some(cognitive_task_to_pb(&task)) }))
    }

    // ── Intent Routing ──

    async fn send_intent(
        &self, request: Request<pb::SendIntentRequest>,
    ) -> Result<Response<pb::SendIntentResponse>, Status> {
        let req = request.into_inner();
        let intent_pb = req.intent.as_ref()
            .ok_or_else(|| Status::invalid_argument("missing intent"))?;
        let intent = intent_message_from_pb(intent_pb)?;
        let intent_id = self.kernel.send_intent(intent).await.map_err(kernel_error_to_status)?;
        Ok(Response::new(pb::SendIntentResponse { intent_id: intent_id_to_bytes(intent_id) }))
    }

    async fn await_intent(
        &self, request: Request<pb::AwaitIntentRequest>,
    ) -> Result<Response<pb::AwaitIntentResponse>, Status> {
        let req = request.into_inner();
        let agent_id = bytes_to_agent_id(&req.agent_id)?;
        let sub_pb = req.subscription.as_ref()
            .ok_or_else(|| Status::invalid_argument("missing subscription"))?;
        let subscription = intent_subscription_from_pb(sub_pb);
        let intent = self.kernel.await_intent(agent_id, subscription)
            .await.map_err(kernel_error_to_status)?;
        Ok(Response::new(pb::AwaitIntentResponse { intent: Some(intent_message_to_pb(&intent)) }))
    }

    // ── Semantic Memory ──

    async fn store_memory(
        &self, request: Request<pb::StoreMemoryRequest>,
    ) -> Result<Response<()>, Status> {
        let req = request.into_inner();
        let agent_id = bytes_to_agent_id(&req.agent_id)?;
        let chunk_pb = req.chunk.as_ref()
            .ok_or_else(|| Status::invalid_argument("missing chunk"))?;
        let chunk = memory_chunk_from_pb(chunk_pb)?;
        self.kernel.store_memory(agent_id, chunk).await.map_err(kernel_error_to_status)?;
        Ok(Response::new(()))
    }

    async fn query_memory(
        &self, request: Request<pb::QueryMemoryRequest>,
    ) -> Result<Response<pb::QueryMemoryResponse>, Status> {
        let req = request.into_inner();
        let agent_id = bytes_to_agent_id(&req.agent_id)?;
        let chunks = self.kernel.query_memory(agent_id, &req.query, req.top_k as usize)
            .await.map_err(kernel_error_to_status)?;
    ...
    async fn forget_memory(
        &self, request: Request<pb::ForgetMemoryRequest>,
    ...
    async fn grant_capability(
        &self, request: Request<pb::GrantCapabilityRequest>,
    ...
    async fn revoke_capability(
        &self, request: Request<pb::RevokeCapabilityRequest>,
    ) -> Result<Response<()>, Status> {
        let req = request.into_inner();
        self.kernel.revoke_capability(bytes_to_capability_cert_id(&req.cert_id)?)
            .await.map_err(kernel_error_to_status)?;
        Ok(Response::new(()))
    }

    async fn delegate_capability(
        &self, request: Request<pb::DelegateCapabilityRequest>,
    ) -> Result<Response<pb::DelegateCapabilityResponse>, Status> {
        let req = request.into_inner();
        let from_agent = bytes_to_agent_id(&req.from_agent)?;
        let to_agent = bytes_to_agent_id(&req.to_agent)?;
        let req_pb = req.requirement.as_ref()
        ...
        let cert_id = self.kernel.delegate_capability(
            from_agent, to_agent, requirement, req.new_scope, req.new_permissions,
        ).await.map_err(kernel_error_to_status)?;
    ...
    async fn get_capabilities(
        &self, request: Request<pb::GetCapabilitiesRequest>,
    ) -> Result<Response<pb::GetCapabilitiesResponse>, Status> {
        let req = request.into_inner();
        let agent_id = bytes_to_agent_id(&req.agent_id)?;
        let cert = self.kernel.get_capabilities(agent_id).await.map_err(kernel_error_to_status)?;
        Ok(Response::new(pb::GetCapabilitiesResponse {
            certificate: Some(pb::CapabilityCertificate {
                ...
                capabilities: cert.capabilities.iter().map(|c| pb::CapabilityProto {
                    cap_type: 0,
    ...
    async fn get_system_status(
        ...
        let status = self.kernel.get_system_status().await.map_err(kernel_error_to_status)?;
    ...
    async fn shutdown(&self, _request: Request<()>) -> Result<Response<()>, Status> {
        self.kernel.shutdown().await.map_err(kernel_error_to_status)?;
        Ok(Response::new(()))
    }
}
